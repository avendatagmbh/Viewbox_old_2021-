    ...
    $.fn.dragtable = function(options) {
                ...
                _D.originalTable.el.find('tr').slice(0, opts.maxMovingRows).each(function(i, v) {
                    ...
                    if (jQuery.browser.msie && jQuery.browser.version.match('^7|^6')) {
                        ...
                        $(this).children().each(function() {
                            var tmp = $(this).height();
                            if (maxCellHeight == null || tmp > maxCellHeight) {
                                maxCellHeight = tmp;
                            }
                        });
       ...
       */
        ...
        return this.each(function() {
            ...
            if (opts.dragaccept) {
                bindTo = bindTo.filter(opts.dragaccept);
            }
            // bind draggable to handle if exists
            if (bindTo.find(opts.dragHandle).size() > 0) {
                bindTo = bindTo.find(opts.dragHandle);
            }
