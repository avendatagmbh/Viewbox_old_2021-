    ...
    public static class Program
    {
        ...
        private static void Alpha() {
            ...
            using (StreamWriter tablesWithError = new StreamWriter(_options.TablesWithErrorPath, true, Encoding.UTF8)) {
                ...
                oraCSB.Server = "sus.ov.otto.de";
                oraCSB.Port = 1521;
                oraCSB.Sid = "OFS2";
                oraCSB.UserId = _options.Oracleuser;
                oraCSB.Password = _options.Oraclepassword;
                oraCSB.Direct = true;
                oracleConnStr = oraCSB.ConnectionString;*/
                    ...
                    using(OracleCommand oraCmd = new OracleCommand()) {
                        ...
                        foreach (string line in lines) {
                            ...
                            if (oraCount != -1) {
                                ...
                                string mySqlLastBinaryKey = null;
                                if (tableExists)
                                {
                                    try {
                                        ...
                                        if (mySqlCount > 0) {
                                            LogInfoMessage("MySql table is not empty, getting last value");
                                            commandStr = String.Format("SELECT HKEY FROM `{0}`.`{1}__{2}` LIMIT {3}, 1", _options.Database, cols[0], cols[1], mySqlCount-1);
                                            using (MySqlConnection mySqlConn = new MySqlConnection(mySqlConnStr))
                                            {
                                                mySqlConn.Open();
                                                using (MySqlCommand mySqlCmd = new MySqlCommand())
                                                {
                                                    ...
                                                    {
                                                        drc.Read();
                                                        mySqlLastBinaryKey = drc.GetString(0);
                                                    }
                                                }
                                            }
                                            LogInfoMessage(string.Format("MySql table is not empty, last value is: [0x{0}]", mySqlLastBinaryKey == null ? "NULL" : mySqlLastBinaryKey));
                                        }
                                    }
                                    catch (Exception ex) {
                                ...
                                if (!tableExists || countDifferences) {
                                    ...
                                    int rowsProcessed = 0;
                                    try {
                                        //if (!string.IsNullOrEmpty(mySqlLastBinaryKey)) {
                                        //    LogInfoMessage("Continuw processing oracle data after 0x" + mySqlLastBinaryKey);
                                        //    commandStr = String.Format("SELECT RAWTOHEX(HAIX1KEY) AS HAIX1KEY, HKEY, HAIX2KEY, HOTIME, HCTIME, HFNO, HSEITEN, HSTAT, HANZN, HWORKDATJH, HWORKDATJ, HWORKDATM, HWORKDATT, HWORKXHH, HWORKXMM, HWORKXSS, HDRDATJH, HDRDATJ, HDRDATM, HDRDATT, HRESERVE FROM \"{0}\".\"{1}\" WHERE RAWTOHEX(HAIX1KEY) > 0x{2} ORDER BY RAWTOHEX(HAIX1KEY)", cols[0], cols[1], mySqlLastBinaryKey);
                                        //} else {
                                        //    LogInfoMessage("Processing oracle data from the beginning");
                                        //    commandStr = String.Format("SELECT RAWTOHEX(HAIX1KEY) AS HAIX1KEY, HKEY, HAIX2KEY, HOTIME, HCTIME, HFNO, HSEITEN, HSTAT, HANZN, HWORKDATJH, HWORKDATJ, HWORKDATM, HWORKDATT, HWORKXHH, HWORKXMM, HWORKXSS, HDRDATJH, HDRDATJ, HDRDATM, HDRDATT, HRESERVE FROM \"{0}\".\"{1}\" ORDER BY RAWTOHEX(HAIX1KEY)", cols[0], cols[1]);
                                        //}
                                        LogInfoMessage("Processing oracle data from the beginning");
                                        commandStr = String.Format("SELECT RAWTOHEX(HKEY) AS HKEY,HZTN,HZEILE132 FROM \"{0}\".\"{1}\"", cols[0], cols[1]);
                                        oraCmd.CommandText = commandStr;
                                        using (OracleDataReader dr = oraCmd.ExecuteReader()) {
                                            ...
                                            //tablestr.Append(",`_row_no_` INT NOT NULL AUTO_INCREMENT, PRIMARY KEY (`_row_no_`), KEY `_d120u_haix1key` (`HAIX1KEY`)");
                                            tablestr.Append(",`_row_no_` INT NOT NULL AUTO_INCREMENT, PRIMARY KEY (`_row_no_`)");
                                            ...
                                            //tablestr.Append(" DEFAULT NULL");
                                            /*
                                            string columnName2 = "_row_no_";
                                            string mysqlTypeString2 = "INT NOT NULL AUTO_INCREMENT";
                                            ...
                                            */
                                            ...
                                            using (MySqlConnection mySqlConn = new MySqlConnection(mySqlConnStr))
                                            {
                                                ...
                                                string insertCmdPrefix = String.Format("INSERT INTO `{0}`.`{1}__{2}` (HKEY, HZTN,HZEILE132) VALUES (", _options.Database, cols[0], cols[1]);
                                                    ...
                                                    try {
                                                        ...
                                                        while (dr.Read()) {
                                                            ...
                                                            try {
                                                                ...
                                                                //    mySqlInsertCmd.Parameters.AddWithValue("@param" + paramId, dr.GetString(1));
                                                                //}
                                                                ...
                                                                //    paramId++;
                                                                //    insertCommand.Append("@param" + paramId);
                                                                //    mySqlInsertCmd.Parameters.AddWithValue("@param" + paramId, dr.GetString(2));
                                                                //}

                                                                for (int fieldNumber = 1; fieldNumber < oracleFieldCount; fieldNumber++)
                                                                {
