...
namespace Utils {
    public class EnhancedCsvReader {
        ...
        public DataTable GetCsvData(int limit, Encoding encoding) {
            ...
            int lines = 0;
            try {
                reader = CreateReader(this.Filename, encoding);
                ...
                while ((currentChar = reader.Read()) != -1 && (limit == 0 || lines < limit)) {
                    ...
                    if (lastColumnCount < 0) {
                        ...
                        for (int i = 0; i < csvLine.Length; i++) {
                            ...
                            if (inTextMode) {
                                if (StringsOptionallyEnclosedBy.HasValue &&
                                    csvLine[i].Equals(this.StringsOptionallyEnclosedBy)) {
                                    inTextMode = !inTextMode;
                                } else {
                                    value.Append(csvLine[i]);
                                }
                ...
                if (reader != null)
                    reader.Close();
                reader = CreateReader(this.Filename, Encoding.UTF7);
                ...
                //while ((csvLine = reader.ReadLine()) != null && (limit == 0 || lines < limit)) {
                //    lines++;
                //    DataRow dr = filename.Rows.Add(new object[] { csvLine.Replace("\r", @"\r").Replace("\n", @"\n") });
                //}
            } finally {
                if (reader != null)
                    reader.Close();
