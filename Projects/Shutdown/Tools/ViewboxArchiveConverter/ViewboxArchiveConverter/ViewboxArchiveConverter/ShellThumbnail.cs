                ...
                if (Path.GetFileName(pidlPath).ToUpper().Equals(Path.GetFileName(file).ToUpper()))
                {
                    ...
                    if (extractImage != null)
                    {
                        ...
                        try
                        {
                            ...
                            // This will fail for such .xls and .doc which does not contains a thumbnail (MS Office can save these files with thumbnail included)
                            extractImage.Extract(ref hBmp);
                        }
                        catch (System.Runtime.InteropServices.COMException ex)
                        {
                            // if general COM error code than retry
                            //if (ex.ErrorCode == -532462766) {
