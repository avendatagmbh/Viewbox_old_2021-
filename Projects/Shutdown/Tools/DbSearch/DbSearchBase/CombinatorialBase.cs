...
namespace DbSearchBase {
    abstract public class CombinatorialBase : IEnumerator {
        ...
        protected CombinatorialBase(Array arrayObjects, int nKlass) {
            ...
            // Always takes the ZERO (FIRST) dimension of the array. There is no problem in manipulation multidimensional arrays.
        ...
        virtual protected void DoArgumentCheck(int nItems, int nKlass) {
            ...
            if (nKlass <= 0)
                throw new ArgumentOutOfRangeException("nKlass", nKlass, "Second parameter (nKlass) to CombinatorialBase constructor must be > 0");

            if (nItems < nKlass)
                throw new ArgumentOutOfRangeException("nKlass", nKlass, "Less than needed objects supplied. Second parameter of CombinatorialBase cannot be greater that the number of objects");
        }
