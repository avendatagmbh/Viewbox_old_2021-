...
using federalGazetteBusiness.Structures.Exceptions;

namespace federalGazetteBusiness.Structures {
    public class ErrorAnalyse {
        #region AnalyseError

        public static void AnalyseError(object response) {
        ...
        private static void ErrorID(int? errId, StringBuilder stringBuilder = null) {
            switch (errId.ToString()) {
                case "20":
                    throw new FederalGazetteResponseError("Validierungsfehler:\n" + stringBuilder);
                case "21":
                    throw new FederalGazetteResponseError("TicketID existiert.\n" + stringBuilder);
                case "22":
                    throw new FederalGazetteResponseError("TicketID ist in Bearbeitung.\n" + stringBuilder);
                case "50":
                    throw new FederalGazetteResponseError("Client konnte nicht gelöscht werden.\nFehler:Authentifizierungsfehler!\n" +
                                        stringBuilder);
                case "51":
                    throw new FederalGazetteResponseError("Client existiert nicht.\n" + stringBuilder);
                case "53":
                    throw new FederalGazetteResponseError(
                        "Die Registrierungsdaten des Veröffentlichungspflichtigen müssen aktualisiert werden.\n" +
                        stringBuilder);
                case "54":
                    throw new FederalGazetteResponseError("Der angegebene Kunde passt nicht zur vorgegebenen Agentur.\n" + stringBuilder);
                case "55":
                    throw new FederalGazetteResponseError("Die angegebene Auftragsnummer ist nicht korrekt.\n" + stringBuilder);
                case "56":
                    throw new FederalGazetteResponseError("Der Veröffentlichungspflichtgen muss sich dem Firmenindex zuordnen.\n" +
                                        stringBuilder);
                case "99":
                    throw new FederalGazetteResponseError(
                        ...
                        stringBuilder);
                case "100":
                    throw new FederalGazetteResponseError("eBundesanziger: Interner Fehler.\nBitt versuchen Sie später erneut.\n" +
                                        stringBuilder);
            }
        }
        #endregion
