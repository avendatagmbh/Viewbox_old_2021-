...
namespace eBalanceKitBusiness.Export {
    public class XbrlExportAnnualStatement {
        ...
        public StringBuilder ExportXmlAnnualStatement(Document document, string file, bool writeXml) {
            ...
            using (XmlWriter xmlWriter = XmlWriter.Create(builder, xmlSettings)) {
                ...
                if (!document.ValueTreeGcd.Root.Values.ContainsKey("genInfo.company.id.idNo")) throw new Exception("Could not find tuple 'genInfo.company.id.idNo'.");
                var idNoNode = document.ValueTreeGcd.Root.Values["genInfo.company.id.idNo"] as XbrlElementValue_Tuple;
                ...
                GenerateXmlContent(document, xmlWriter, entityScheme, entityId);
        ...
        private void WriteGCD(XmlWriter xmlWriter, Document document) {
        ...
        private void WriteGAAP(XmlWriter xmlWriter, Document document, XmlNamespace gaapNamespace) {
            ...
            foreach (var balanceList in document.BalanceLists) 
                InsertAccountBalances(xmlWriter, balanceList, gaapNamespace, document);
        ...
        //private StringBuilder GetXmlAnnualStatement(Document document, string file) {
        //    var result = new StringBuilder();
        //    var xmlSetting = new XmlWriterSettings {Indent = true, IndentChars = "    ", Encoding = Encoding.UTF8};

        //    using (XmlWriter xmlWriter = XmlWriter.Create(result, xmlSetting)) {
        //        var innerWriterPropInfo = xmlWriter.GetType().GetProperty("InnerWriter",
        //                                                                  BindingFlags.NonPublic |
        //                                                                  BindingFlags.Instance);
        //        var innerWriter = (XmlWriter) innerWriterPropInfo.GetValue(xmlWriter, null);
        //        var encodingFieldInfo = innerWriter.GetType().GetField("encoding",
        //                                                               BindingFlags.NonPublic | BindingFlags.Instance);
        //        if (encodingFieldInfo != null) encodingFieldInfo.SetValue(innerWriter, Encoding.Unicode);
        //        //GenerateXmlContent(document, xmlSetting, xmlWriter);
        //        xmlWriter.WriteEndElement();
        //        xmlWriter.WriteEndDocument();
        //    }
                ...
                case Taxonomy.Enums.XbrlElementValueTypes.SingleChoice:
                    ...
                    if (value.Value != null) {
                        ...
                        if (value.Element.IsMandatoryField) {
                            ...
                            //Dirty hack for elster client if no shareholder is present. whether its important in case of 
