...
namespace Utils.Commands
{
    ...
    public class CommandBridge : FrameworkElement
    {
        ...
        private static readonly Dictionary<WeakReference, CommandedElementData> _elements;
        ...
        private static void OnCanExecutePropertyValuePropertyChanged(DependencyObject d,
        ...
        private static void OnCanNotExecutePropertyValuePropertyChanged(DependencyObject d,
                                                                        DependencyPropertyChangedEventArgs e)
        {
            FrameworkElement element = d as FrameworkElement;
            if (element != null)
        ...
        public static readonly DependencyProperty CommandActionProperty =
            DependencyProperty.RegisterAttached("CommandAction", typeof (CommandAction), typeof (CommandBridge),
                                                new PropertyMetadata(CommandAction.Disable,
                                                                     OnCommandActionPropertyChanged));
        ...
        private static void UpdateCanExecute(UIElement element)
        {
            ...
            {
                (element).Visibility = (canExecute) ? Visibility.Visible : Visibility.Collapsed;
            }
            ...
            if (info != null)
            {
                try
                {
                    ...
                    if (info.PropertyType.Equals(typeof (bool)) && data.CanExecutePropertyValue == null &&
                        data.CanNotExecutePropertyValue == null)
                    {
                    ...
                    else
                    {
                        ...
                        object value;
                        if (canExecute)
                            value = ConvertValue(data.CanExecutePropertyValue, info.PropertyType, data.Converter,
                                                 data.ConverterParameter);
                        else
                            value = ConvertValue(data.CanNotExecutePropertyValue, info.PropertyType, data.Converter,
                                                 data.ConverterParameter);
        ...
        private static object ConvertValue(object propertyValue, Type targetType, IValueConverter converter,
                                           object converterParameter)
        {
            try
            {
                ...
                if (converter != null)
                {
                    return converter.Convert(propertyValue, targetType, converterParameter,
                ...
                else
                {
                        ...
                        targetType.GetGenericTypeDefinition().Equals(typeof (Nullable<>)))
                    {
                        ...
                        {
                            value = Convert.ChangeType(propertyValue, t1, Thread.CurrentThread.CurrentCulture);
                    ...
                    else
                    {
                        ...
                        return Convert.ChangeType(propertyValue, targetType, Thread.CurrentThread.CurrentCulture);
                    }
                }
            }
            catch
        ...
        private static void CreateHandler(UIElement element, CommandedElementData elementData)
        {
            ...
            if (eventInfo == null)
                throw new InvalidOperationException(
                    string.Format("The element \"{0}\" dosn't contain an event with the name \"{1}\"!",
                                  element.GetType().Name, elementData.EventName));
            Type handlerType = eventInfo.EventHandlerType;
            MethodInfo invokeMethod = handlerType.GetMethod("Invoke");
            ParameterInfo[] parms = invokeMethod.GetParameters();
            Type eventArgsType = parms[1].ParameterType;
            // create generic handler
            var genericHandlerType = typeof (GenericHandler<>).MakeGenericType(new[] {eventArgsType});
            var handler = Activator.CreateInstance(genericHandlerType,
                                                   new[] {elementData.Command, elementData.CommandParameter});
                // eventArgsType)
        ...
        private class CommandedElementData : IDisposable
        {
            ...
            public void Dispose()
            {
                ...
                CanExecutePropertyValue = null;
                CanNotExecutePropertyValue = null;
                Converter = null;
        ...
        private class GenericHandler<TEventArgs>
        {
            private readonly ICommand _command;
            private readonly object _commandParams;
