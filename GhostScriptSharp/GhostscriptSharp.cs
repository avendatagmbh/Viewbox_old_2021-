...
namespace GhostscriptSharp
{
    ...
    public class GhostscriptWrapper
    {
        ...
        private static readonly string[] ArgsPsPdf = new[]
                                                         {
                                                             "-q",
                                                             "-dQUIET",
                                                             "-dPARANOIDSAFER",
        ...
        /// </summary>
        public static void ConvertPdfToThumbs(string inputPath, string outputPath, int page, int width = 0,
                                              int height = 0)
        {
            ConvertPdfToThumbs(inputPath, outputPath, page, page, width, height);
        }

        /// <summary>
        ///   Generates a collection of thumbnail jpgs for the pdf at the input path starting with firstPage and ending with lastPage. Put "%d" somewhere in the output path to have each of the pages numbered
        /// </summary>
        public static void ConvertPdfToThumbs(string inputPath, string outputPath, int firstPage, int lastPage,
                                              int width = 0, int height = 0)
        {
            if (IntPtr.Size == 4)
                GhostScript32.CallAPI(GetArgs(inputPath, outputPath, firstPage, lastPage, width, height));
            else
                GhostScript64.CallAPI(GetArgs(inputPath, outputPath, firstPage, lastPage, width, height));
        }

        public static void ConvertPsToPdf(string inputPath, string outputPath)
        {
            var args = new ArrayList(ArgsPsPdf);
            args.Add(String.Format("-sOutputFile={0}", outputPath));
            args.Add(inputPath);
            var parameterArgs = (string[]) args.ToArray(typeof (string));
            if (IntPtr.Size == 4)
                GhostScript32.CallAPI(parameterArgs);
            else
                GhostScript64.CallAPI(parameterArgs);
        }
                                        ...
                                        int height)
        {
            ...
            var s = new GhostscriptSettings();
            s.Device = GhostscriptDevices.jpeg;
            s.Page.Start = firstPage;
            s.Page.End = lastPage;
            s.Resolution = new Size(72, 72);
        ...
        {
            var args = new ArrayList(Args);
            if (settings.Device == GhostscriptDevices.UNDEFINED)
            {
                throw new ArgumentException("An output device must be defined for Ghostscript",
                                            "GhostscriptSettings.Device");
            }
            if (settings.Page.AllPages == false &&
                (settings.Page.Start <= 0 && settings.Page.End < settings.Page.Start))
            {
...
namespace GhostscriptSharp.Settings
{
    ...
    /// </summary>
    /// <remarks>
    ///   Missing 11x17 as enums can't start with a number, and I can't be bothered to add in logic to handle it - if you need it, do it yourself.
    /// </remarks>
